...
def covert_native_list_to_collection(any_list):
    points_collection_result = []
    iterator_first = list(any_list)
    xx = -1
    yy= -1
    idx=0
    for item in iterator_first:
        idx+=1
        if(idx%2==1):
            xx=int(item)
        else:
            yy=int(item)
            point = (xx,yy)   
            points_collection_result.append(point)

    return points_collection_result

points_first_collection = covert_native_list_to_collection(first)
points_second_collection = covert_native_list_to_collection(second)
import PIL.ImageDraw as ImageDraw
import PIL.Image as Image

image = Image.new("RGB", (640, 480))
draw = ImageDraw.Draw(image)
draw.polygon(points_first_collection)
draw.polygon(points_second_collection)
image.save("c:\Python\my_image.jpg", 'JPEG')
image.show()